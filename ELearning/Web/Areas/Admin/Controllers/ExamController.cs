	...
	public class ExamController : AdminBaseController
	{
		...
		public async Task<IActionResult> ListStudentOfTakingExam(int id)
		{
			var user = await _db.Users
				.Join(_db.GroupDetails, u => u.Id, gd => gd.UserId, (u, gd) => new { u, gd })
				.Join(_db.Groups, ugd => ugd.gd.GroupId, g => g.Id, (ugd, g) => new { ugd, g })
				.Join(_db.HandOutExam, ug => ug.g.Id, he => he.GroupId, (ug, he) => new { ug, he })
				.Where(x => x.he.ExamId == id)
				.Select(x => new ListStudentTakeExamVM
				{
					Mssv = x.ug.ugd.u.MSSV,
					FullName = x.ug.ugd.u.FullName,
					Email = x.ug.ugd.u.Email,
					GroupName = x.ug.g.GroupName,
					GroupId = x.ug.g.Id,
				}).ToListAsync();


			var result = _repo.GetAll<Result>(Result => Result.ExamId == id)
								.ToList();

			user = user.Select(x =>
			{
				x.NumTSC = result.Where(r => r.UserId == x.Id && r.ExamId == id).First().NumTSC;
				x.TestScores = result.Where(r => r.UserId == x.Id && r.ExamId == id).First().TestScores;
				x.StartTime = result.Where(r => r.UserId == x.Id && r.ExamId == id).First().StartTime;
				x.TotalWorkingTime = result.Where(r => r.UserId == x.Id && r.ExamId == id).First().TotalWorkTime;
				return x;
			}).ToList();
